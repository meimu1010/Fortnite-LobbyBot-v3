...
class PartyPrivacy(rebootpy.Enum):
    PUBLIC = {
        'partyType': 'Public',
...
class FindUserMatchMethod(rebootpy.Enum):
    FULL = 'full'
    CONTAINS = 'contains'
    STARTS = 'starts'
    ENDS = 'ends'
...
class FindUserMode(rebootpy.Enum):
...
class MyMessage:
    def __init__(self, client: Clients, message: Messages, *,
        ...
        self.user_type = (
            ...
            'owner'
            if not isinstance(self.message, (discord.Message, rebootpy.message.MessageBase)) else
    ...
    def is_friend_message(self) -> bool:
        return isinstance(self.message, rebootpy.FriendMessage)

    def is_party_message(self) -> bool:
        return isinstance(self.message, rebootpy.PartyMessage)

    async def reply(self, content: str) -> None:
        content = str(content)
        if isinstance(self.message, rebootpy.message.MessageBase):
...
async def add_to_list(attr: str, message: MyMessage, user: rebootpy.User):
...
async def remove_from_list(attr: str, message: MyMessage, user: rebootpy.User):
    client = message.client
...
class DefaultCommands:
    ...
    async def invite_list_users(command: Command, client: 'Client', message: MyMessage) -> None:
        users = [user for user in client.invitelist
                 if isinstance(user, rebootpy.Friend)]
        if len(users) == 0:
            return
        ...
        if len(message.args) >= 5:
            ...
            try:
                background_colors = getattr(
                    rebootpy.KairosBackgroundColorPreset,
                ...
                await message.reply(
                    client.l(
                        'must_be_one_of',
                        client.l('color'),
                        [i.name for i in rebootpy.KairosBackgroundColorPreset]
                    )
                )
                return

        avatar = rebootpy.Avatar(
    ...
    async def hide(command: Command, client: 'Client', message: MyMessage) -> None:
        if len(message.args) < 2:
            ...
            try:
                ...
                if isinstance(e, rebootpy.HTTPException):
                    client.debug_print_exception(e)
                else:
